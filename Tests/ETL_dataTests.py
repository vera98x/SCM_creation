...
#     df_filter = df_complete[(df_complete["basic|drp_act"] == 'D') | (df_complete["basic|drp_act"] == 'A') | (df_complete["basic|drp_act"] == 'V')]
#     df_K_A = df_filter
#     df_K_V = df_filter.copy()
#     df_K_A["basic|drp_act"] = df_K_A["basic|drp_act"].replace('D','K_A')
#     df_K_A["basic|drp_act"] = df_K_A["basic|drp_act"].replace('A','K_A')
#     df_K_A["basic|drp_act"] = df_K_A["basic|drp_act"].replace('V','K_A')
#
#     df_K_V["basic|drp_act"] = df_K_V["basic|drp_act"].replace('D','K_V')
#     df_K_V["basic|drp_act"] = df_K_V["basic|drp_act"].replace('A','K_V')
#     df_K_V["basic|drp_act"] = df_K_V["basic|drp_act"].replace('V','K_V')
#
#     x1 = datetime.datetime(2019, 1, 3).date()
#     # x2 = datetime.datetime(2019, 5, 10).date()
#     treinserie = "500E"
#     treinnr = "501"
#
#     ka = df_K_A[(df_K_A['date'] == x1) & (df_K_A['basic_treinnr_treinserie'] == treinserie)]
#     kv = df_K_V[(df_K_V['date'] == x1) & (df_K_V['basic_treinnr_treinserie'] == treinserie)]
#
#     print("LEN!:", len(df_K_A), len(df_K_V))
#     print(ka[(ka['basic|treinnr'] == treinnr)])
#     print(kv[(kv['basic|treinnr'] == treinnr)])
#
#     df_res = df_complete[~(df_complete["basic|drp_act"] == 'D') & ~(df_complete["basic|drp_act"] == 'A') & ~(df_complete["basic|drp_act"] == 'V')]
...
def getNumberOfInterventions():
    d = {'prl_rijw_eerst_rijweg_van': ['UF', "PD", "AF", "GH"],
         'prl_rijw_laatst_rijweg_van': ['UF', "PD", "AF", "GH"],
         'prl_rijw_eerst_rijweg_naar': [np.NaN, "CD", "EF", "GH"],
         'prl_rijw_laatst_rijweg_naar': [np.NaN, "CD", "EF", "GH"],
         'prl_rijw_eerst_rijweg_dwangnr': [-1, -1, -1, -1],
         'prl_rijw_laatst_rijweg_dwangnr': [-1, -1, -1, -1] }
    ...
    pd.set_option('display.max_columns', None)
    print(df.loc[(df['intervened'] == False)])
    return (df['intervened'] == False).sum(), (df['intervened'] == True).sum()

print(getNumberOfInterventions())